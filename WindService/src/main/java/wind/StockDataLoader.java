...
public class StockDataLoader {
	...
	private static final String SELECT_STOCK_SQL=" (select t1.stockCode, t1.ktype, t1.w_time, t1.open, t1.high, t1.low, t1.close, t1.volume, t1.amt, t1.ma5, t1.ma10, t1.ma20, t1.adjfactor,mx1.adjfactor as max_adjfactor,createdAt,updatedAt "
			+ " from ws_kdata t1  "
			+ " join (select stockCode, adjfactor, max(yeardate) from ws_kdata group by stockCode) mx1 "
			...
			+ " from ws_kdata t2  "
			+ " join (select stockCode, adjfactor, max(yeardate) from ws_kdata group by stockCode) mx2 "
			+ " on t2.stockCode = mx2.stockCode "
			+ " where t2.stockCode = ? and t2.ktype = ? and t2.w_time >= ? limit 150) ";
